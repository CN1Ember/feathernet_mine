...
def report_format(collected_nodes):
    ...
    total_conv_flops = df['ConvFlops'].sum()
    ...
    total_df = pd.Series([total_parameters_quantity, total_memory,
                          ...
                          total_conv_flops, total_duration,
                          mread, mwrite, total_memrw],
                         index=['params', 'memory(MB)', 'MAdd', 'Flops',
                                'ConvFlops', 'duration[%]',
    ...
    df['ConvFlops'] = df['ConvFlops'].apply(lambda x: '{:,}'.format(x))

    summary = str(df) + '\n'
    summary += "=" * len(str(df).split('\n')[0])
    summary += '\n'
    summary += "Total params: {}B\n".format(
        round_value(total_parameters_quantity))
    ...
    summary += "Total Flops(Conv Only): {}Flops\n".format(
        round_value(total_conv_flops))
