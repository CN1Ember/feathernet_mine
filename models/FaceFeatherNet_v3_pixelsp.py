...
def FaceFeatherNetA_v3(se=False, width_mult=1.0):
    model = FaceFeatherNetFT_v3(se = se, width_mult=width_mult)
    return model
...
if __name__ == "__main__":
    ...
    print(model)
    x = torch.from_numpy(np.zeros((1,1,112,112),np.float32))
    result = model(x)
