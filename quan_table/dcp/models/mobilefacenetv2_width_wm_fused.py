...
class Mobilefacenetv2_width_wm_fuse(nn.Module):
    def __init__(self, embedding_size=512, mask_train=False, pruning_rate=0):
        super(Mobilefacenetv2_width_wm_fuse, self).__init__()
        ...
        self.conv1 = nn.Conv2d(3, self.inplanes, kernel_size=3, stride=2, padding=1, bias=True)
        self.relu1 = nn.ReLU(inplace = True)
        # self.relu1 = nn.PReLU(self.inplanes)
        self.conv2 = nn.Conv2d(self.inplanes, self.inplanes, kernel_size=3, groups=64, stride=1, padding=1, bias=True)
        self.relu2 = nn.ReLU(inplace = True)
        # self.relu2 = nn.PReLU(self.inplanes)

        self.layers = self._make_layer(Bottleneck_mobilefacenetv2, block_setting, mask_train, pruning_rate=pruning_rate)

        # without pruning fc
        self.last_planes = 512
        self.conv3 = nn.Conv2d(self.inplanes, self.last_planes, kernel_size=1, stride=1, padding=0, bias=True)
        self.relu3 = nn.ReLU(inplace = True)
        # self.relu3 = nn.PReLU(self.last_planes)
        if mask_train:
...
if __name__ == "__main__":
     model = Mobilefacenetv2_width_wm_fuse(embedding_size=128, pruning_rate=0)
     # print(model)
