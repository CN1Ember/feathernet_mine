...
class Experiment(object):
    ...
    def _load_pretrained(self):
        ...
        if self.settings.pretrained is not None:
            ...
            # self.aux_fc = self.checkpoint.load_state(self.aux_fc, aux_fc_state)
            self.logger.info("load aux_fc state finished!!")

            # self.logger.info("val_acc={}".format(check_point_params['val_acc']))
            self.logger.info("epoch={}, val_acc={}".format(check_point_params['epoch'], check_point_params['lfw_accuracy']))
