...
from dcp.models.mobilefacenetv2_width_wm import Mobilefacenetv2_width_wm
import numpy as np
...
def save_weight_table(net, calibration_path):
    ...
    for name, layer in net.named_modules():
        ...
        if isinstance(layer, nn.Conv2d):
            ...
            weight_ = layer.weight.data
            bn_scale = bn_model.weight.data
            bn_bias = bn_model.bias.data
            bn_mean = bn_model.running_mean.data
            bn_var = bn_model.running_var.data

            # var_scale = var_scale.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * self.weight
            fused_weight = bn_scale / torch.sqrt(bn_var + 1e-05) 
            fused_bias =   - bn_mean * bn_scale  / torch.sqrt(bn_var + 1e-05) + bn_bias
            fused_weight = fused_weight.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * weight_
...
def main(args):
    print(args)
    net = Mobilefacenetv2_width_wm(embedding_size=128, pruning_rate=0)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--model_path', type=str, help='model_path',
                        default='/home/xiezheng/lidaiyuan/Face_integration/train_log/Soft-finetuning-p0-noquan-lr0.0001-[30]-1224-data_aug-ft128/check_point/checkpoint_1.pth')
    parser.add_argument('--calibration_path', type=str, help='calibration_path',
                        default='/home/xiezheng/lidaiyuan/Face_integration/train_log/Soft-finetuning-p0-noquan-lr0.0001-[30]-1224-data_aug-ft128/check_point/checkpoint_1.pth_w.table')
