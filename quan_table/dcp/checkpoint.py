...
class CheckPoint(object):
    ...
    def save_model(self, model, best_flag=False, index=0, tag=""):
        ...
        model = model.state_dict()    # save model state_dict()
        if best_flag:
            if tag != "":
    ...
    def save_aux_model(self, model, aux_fc):
        ...
        if isinstance(aux_fc, nn.DataParallel):
            check_point_params["aux_fc"] = aux_fc.module.state_dict()
        else:
            check_point_params["aux_fc"] = aux_fc.state_dict()
        torch.save(check_point_params, os.path.join(self.save_path, "model_with_aux_fc.pth"))


    ############# insightface_dcp ###############
    def save_face_model(self, model, best_flag=False):
        """
        :params model: model
        :params optimizer: optimizer
        :params epoch: training epoch
        ...
        if best_flag:
            ...
            torch.save(check_point_params, os.path.join(self.save_path, "best_model_without_aux_fc.pth"))


    def save_face_checkpoint(self, model, aux_fc, optimizer, epoch, val_acc, scheduler, index=0):
        """
        :params model: model
        :params optimizer: optimizer
        :params epoch: training epoch
        ...
        check_point_params['scheduler'] = scheduler.state_dict()
        check_point_params['epoch'] = epoch
        check_point_params['val_acc'] = val_acc
