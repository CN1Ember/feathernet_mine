...
# from insightface_v2.insightface_data.data_pipe import RecordDataset
from insightface_v2.model.inception_resnet import InceptionResNetV2
...
def train_net(args):
    ...
    # dataset = RecordDataset('/mnt/ssd/datasets/Faces/train/ms1m-retinaface-t1/train.rec')
    ...
    if args.network == 'r34':
        ...
        model = Mobilefacenetv2_v2_depth(embedding_size=args.emb_size, blocks=args.block_setting)
    ...
    elif args.network == 'InceptionResNetV2':
    ...
    if args.pretrained != '' :
        ...
        metric_fc.load_state_dict(metric_fc_state)
        # metric_fc_state = pretrained_state["metric_fc"]
        # if isinstance(metric_fc, nn.DataParallel):
        #     metric_fc.module.load_state_dict(metric_fc_state)
        # else:
        #     metric_fc.load_state_dict(metric_fc_state)

        # lfw_acc = pretrained_state["lfw_best_acc"]
        # start_epoch = pretrained_state['epoch']
        # logger.info("epoch={}, lfw acc={:4f}".format(pretrained_state['epoch'], lfw_acc))
    ...
    if checkpoint is None or checkpoint == '':
        if args.network in ['mobilefacenet_v1', 'mobilefacenet_v2', 'zq_mobilefacenet', 'mobilenet_v3']:
            ...
            if args.optimizer == 'sgd':
                ...
                #                        {'params': metric_fc.bias, 'weight_decay': 4e-4},
                ...
                # logger.info("metric_fc.bias={}".format(metric_fc.bias))

                optimizer = optim.SGD([{'params': paras_wo_bn, 'weight_decay': 4e-5},
                                       {'params': [metric_fc.weight], 'weight_decay': 4e-4},
                                       {'params': [metric_fc.bias], 'weight_decay': 0,
                                        'lr': 2 * args.lr, 'is_bias': True},
                                       # {'params': metric_fc.parameters(), 'weight_decay': 4e-4},
                                       {'params': paras_only_bn, 'weight_decay': 0},
                                       {'params': paras_bias, 'weight_decay': 0}],
                                      lr=args.lr, momentum=args.mom, weight_decay=args.weight_decay, nesterov=True)
                ...
                logger.info("bias fixed !!!")
                logger.info(optimizer)
            elif args.optimizer == 'adam':
                optimizer = optim.Adam([{'params': model.parameters()}, {'params': metric_fc.parameters()}],
                                       lr=args.lr, weight_decay=args.weight_decay)
            else:
                assert False, "not support args.optimizer={}".format(args.optimizer)

        else:
            if args.optimizer == 'sgd':
                logger.info('InceptionResNetV2')
    ...
    for epoch in range(start_epoch, args.end_epoch):
        ...
        # tpr = val_verification(args, model, verfication, verfication_issame, fpr=10e-3)
        ...
        # else:
        save_checkpoint(args, epoch, model, metric_fc, optimizer, scheduler, tpr_best)
