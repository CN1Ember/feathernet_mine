...
class Mobilefacenetv2_v2_depth_width(nn.Module):

    def __init__(self, embedding_size=512, blocks=[2,8,16,4], fc_type='gnap', width_mult=1.0):
        super(Mobilefacenetv2_v2_depth_width, self).__init__()
        ...
        self.layers = self._make_layer(Bottleneck_mobilefacenetv2, Mobilefacenetv2_bottleneck_setting, width_mult=width_mult)
        ...
        if fc_type == 'gdc':
            self.conv3 = nn.Conv2d(self.last_planes, self.last_planes, kernel_size=7, groups=self.last_planes, stride=1, padding=0, bias=False)
...
if __name__ == "__main__":
    ...
    # model = Mobilefacenetv2_v2_depth(blocks=[2, 8,16, 4], embedding_size=512, fc_type="gdc", width_mult=1.1)

    # model = Mobilefacenetv2(blocks=[2,8,16,6], embedding_size=512, fc_type="gnap")  # FLOPs: 946M
    # model = Mobilefacenetv2(blocks=[3, 8, 16, 6], embedding_size=512, fc_type="gnap")  # FLOPs: 1001M
    # model = Mobilefacenetv2(blocks=[3, 8, 16, 5], embedding_size=512, fc_type="gnap")  # FLOPs: 994M, Params size (MB): 7.31
    ...
    print("model flops={}".format(sum(flops)))
