...
class CASIA(Dataset):
    def __init__(self,data_flag = None, transform=None, phase_train=True, data_dir=None,phase_test=False,add_mask = True,feat_map_shape = (7,7)):
    ...
    def __getitem__(self, idx):
        ...
        fmap_mask2 = np.ones((14,14), np.uint8) * label
        # fmap_mask2 = np.bitwise_and(fmap2,self.mask_np)

        if label == 0:
            cv2.imwrite('./test.jpg',fmap_mask * 255)
        ...
        if random.randint(0,9) < 5:
            ...
            # fmap_mask2 = np.fliplr(fmap2).copy()
        ...
        if self.phase_train:
            return depth,label,fmap_mask,fmap_mask2
        else:
