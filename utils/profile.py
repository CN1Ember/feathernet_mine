...
def calc_flops(model, input_size):
    ...
    if '0.4.' in torch.__version__:
        if USE_GPU:
            input = torch.cuda.FloatTensor(torch.rand(2, 1, input_size, input_size).cuda())
        else:
            input = torch.FloatTensor(torch.rand(2, 1, input_size, input_size))
    else:
        input = Variable(torch.rand(2, 1, input_size, input_size), requires_grad=True)
    _ = model(input)
...
def count_params(model, input_size=112):
    # param_sum = 0
