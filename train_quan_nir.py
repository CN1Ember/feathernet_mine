...
from ncnn_quantization import Conv2d_Ncnn_int8, replace_layer_by_unique_name
...
parser.add_argument('--table_path', type=str, default='featherneta_nir_032001_147_activation.table')
parser.add_argument('--data_flag', type=str, default='exp_train_set_nir_21031802_exp_20210325205339', help='use gpus training eg.--gups 0,1')
...
logger = Logger('./logs_visualization/logs_nose_nir_0510_mimic')
...
def main():
    ...
    activation_value_list = get_activation_value(args.table_path, activation_value_list)

    # replace convolutional layers
    model = replace(model, activation_value_list)
    ...
    train_dataset = CASIA(
        ...
        transforms.Compose([
            ...
            ColorAugmentation(),
            normalize,
        ]), phase_train=True, add_mask=False)
    print('val load')
    val_dataset = CASIA(
        data_flag,
        transforms.Compose([
        ...
        transforms.ToTensor(),
        normalize,
    ]), phase_train=False, phase_test=args.phase_test, add_mask=False)
    ...
    for epoch in range(args.start_epoch, args.start_epoch+args.epochs):
        ...
        if not os.path.exists(args.save_path + '_quan'):
            os.mkdir(args.save_path + '_quan')
        save_name = '{}_quan/{}_{}_best.pth.tar'.format(args.save_path, args.model_name, epoch) if is_best else \
            '{}_quan/{}_{}.pth.tar'.format(args.save_path, args.model_name, epoch)
...
def get_activation_value(table_path, activation_value_list):
...
def replace(model, activation_value_list):
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d):
            ...
            print("name: {}, activation_value_list[name]: {}".format(name, activation_value_list[name]))
    # print(model)
    # assert False
    return model
