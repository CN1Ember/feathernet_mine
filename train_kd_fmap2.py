...
parser.add_argument('--tarch', '-ta', metavar='ARCH', default='resnet18',
                    ...
                    help='models architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet18)')
parser.add_argument('--config', default='cfgs/FaceFeatherNetA-kd-fmap.yaml')
...
parser.add_argument('--data_flag', type=str, default='exp_train_set_210226_exp_20210226212105', help='use gpus training eg.--gups 0,1')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, teacher, criterion, criterion_ft, optimizer, epoch)
...
def train(train_loader, model, teacher, criterion, fmap_criterion, optimizer, epoch):
    ...
    for i, (input, target, target_map2) in enumerate(train_loader):
        ...
        # compute output
        output,fm_ouput2 = model(input_var)
