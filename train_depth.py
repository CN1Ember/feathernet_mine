...
from read_data.read_data_for_depth import CASIA
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet18',
                    ...
                    help='models architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet18)')
parser.add_argument('--config', default='cfgs/ResNet-18.yaml')
...
parser.add_argument('--data_flag', type=str, default='train_set_21030601_20210307104514', help='use gpus training eg.--gups 0,1')
parser.add_argument('--optimize', type=str, default='SGD', help='SGD,Adam')
...
def main():
    global args, best_prec1, USE_GPU, device, mask_np
    ...
    train_dataset = CASIA(
        ...
        transforms.Compose([
            ...
            ColorAugmentation(),
            normalize,
        ]), phase_train=True, add_mask=args.add_mask)
    print('val load')
    val_dataset = CASIA(
        data_flag,
        transforms.Compose([
        ...
        transforms.ToTensor(),
        normalize,
    ]), phase_train=False, phase_test=args.phase_test, add_mask=args.add_mask)
    ...
    logger = Logger('./logs_visualization/logs_{}_{}_{}_{}_{}_{}'.format(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()), args.arch, args.add_mask, args.batch_size, args.lr, args.optimize ,args.data_flag))
    
    if args.evaluate:
        validate(val_loader, model, criterion, args.start_epoch, logger)
        return
    else:
        print(model)
    ...
    # valid before train
    prec1 = validate(val_loader, model, criterion, -1, logger)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        prec1 = validate(val_loader, model, criterion, epoch, logger)
        ...
        # print('%s_%d_%f_%s'%(args.save_path, args.batch_size, args.lr))
        if not os.path.exists('./{}_{}_{:.5f}_{}_{}_mask_{}/'.format(args.save_path, args.batch_size, args.lr, args.optimize ,args.data_flag ,args.add_mask)):
            os.mkdir('./{}_{}_{:.5f}_{}_{}_mask_{}/'.format(args.save_path, args.batch_size, args.lr, args.optimize ,args.data_flag ,args.add_mask))
            
        save_name = './{}_{}_{:.5f}_{}_{}_mask_{}/{}_{}_best.pth.tar'.format(args.save_path, args.batch_size, args.lr, args.optimize ,args.data_flag ,args.add_mask , args.model_name, epoch) if is_best else \
            './{}_{}_{:.5f}_{}_{}_mask_{}/{}_{}.pth.tar'.format(args.save_path, args.batch_size, args.lr, args.optimize ,args.data_flag, args.add_mask, args.model_name, epoch)
...
def train(train_loader, model, criterion, optimizer, epoch, logger):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            with open('logs/{}_{}_{}_{}.log'.format(time_stp, args.arch, args.batch_size, args.lr), 'a+') as flog:
                ...
                flog.write('{}\n'.format(line))
    # =================== Log =============================
    info = {'train_loss': losses.avg, 'train_accuracy': top1.avg, 'learning_rate': lr}
